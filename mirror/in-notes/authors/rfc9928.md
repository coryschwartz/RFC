...
BBUs can be cabled to RUs directly or via a Layer 2 switched network.
...
~~~aasvg
     ...
     +--------+
~~~
{: #bb_connected_to_ru title="3GPP RAN Where RUs Are Cabled Directly to BBUs" artwork-align="center"}

In {{bb_connected_to_ru}}, the BBU is directly cabled to a set of RUs, and the
BBU can recognize the relationship between RUs and Cell/Sectors
based on the cabling between the RUs and antennas.

When BBUs and RUs are connected via a Layer 2 switched network,
