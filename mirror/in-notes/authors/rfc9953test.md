...
- Update [PRE-RFC9952] to [RFC9952] (i.e., remove "PRE-"). Note that we
updated [I-D.ietf-core-coap-dtls-alpn] to [PRE-RFC9952] and will make the
final updates in RFCXML.

- Update citation tags for Internet-Drafts
-->
...
proxies, which provide an additional level of caching; and reuse of data
...
<!--[rfced] Is "authoritive name server" correct or was "authoritative
name server" perhaps intended? We ask as other RFCs use
"authoritative" when referring to a name server.
...
As long as each docpath-segment is a length 0 and 24 octets, it is easily transferred into the path
...
) Some lines in Figure 1 are too long for the TXT output. This figure is
marked as artwork, so it needs to have a width of 72 characters or less. How
may we revise this figure to fit these parameters? Please check out the
following test files and let us know if this would work.

Test files:
.xml
.txt
.html
-rfcdiff.html (diff showing changes in Figure 1)


) We have updated the blocks in Sections 3.2, 3.2.1, 4.2.3, and 4.3.3 to be
marked as sourcecode. We set the type for the block in Section 3.2 as
"abnf". Please let us know if the language should be set for the other
sourcecode blocks. For example, should the ones in Section 3.2.1 be marked as
type "dns-rr"? If the current list of preferred values for languages (see link
below) contain an applicable language, feel free to let us know. Also, it is
acceptable to leave the language not set.

List of sourcecode types:
https://www.rfc-editor.org/rpc/wiki/doku.php?id=sourcecode-types


) The blocks in Section 4.3.3 are too long for the txt output. We marked these
as sourcecode, so they should have a width of 69 characters or less. The long
lines are currently 70 characters. Would moving all the lines with semicolons
over to the left one space (in just this section section or in all the
sourcecode in the document) be a good solution?  Please review and let us know
how to either shorten or wrap these lines to fit the line-length limits.
